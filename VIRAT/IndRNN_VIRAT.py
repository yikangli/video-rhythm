...
root = './'
split = ['App_VIRAT_train_list.txt', 'App_VIRAT_test_list.txt']
...
for k in range(1):
    ...
    for epoch in range(start_epoch, start_epoch+750):
        ...
        for batch_idx, (app_inputs, app_lengths, targets) in enumerate(trainloader):
            ...
            Regular1 += regu1.item()
            Pen += penalty
            progress_bar(batch_idx, len(trainloader), 'Cross Entropy Loss: %.3f, Regu1: %.3f, Sparsity: %.3f' % (en_loss/float(batch_idx+1),Regular1,Pen/float(batch_idx+1)))
