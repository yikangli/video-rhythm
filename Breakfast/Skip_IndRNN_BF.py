...
class VENet(nn.Module):
    def __init__(self):
        ...
        self.mlp_pooling1 = nn.Linear(num_hidden1,num_hidden1)
    ...
    def forward(self, app_x, app_lengths, hidden_app=None):
        ...
        Pooled_output = torch.sigmoid(self.prob_app(X_pooled))
...
for k in range(1):
    ...
    net = VENet()
    if resume == True:
        with open('./checkpoint/ckpt_best_skipindrnn_BF.pkl','rb') as f:
    ...
    scheduler = lr_scheduler.ReduceLROnPlateau(optimizer, mode='min', factor=0.6, patience=30, verbose=True)
        ...
        if numpy.mod(epoch, 1) == 0:
            ...
            if acc > best_acc:
                ...
                if not os.path.isdir('checkpoint'):
                    os.mkdir('checkpoint')
                with open('./checkpoint/ckpt_best_skipindrnn_BF.pkl','wb') as f:
            ...
            if epoch == 300:
                ...
                if not os.path.isdir('checkpoint'):
                    os.mkdir('checkpoint')
                with open('./checkpoint/ckpt_best_skipindrnn_BF_last.pkl','wb') as f:
                    pickle.dump(net.state_dict(), f)
                best_acc = acc
